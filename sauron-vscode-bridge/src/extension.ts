...
export async function handleIncomingHandoffWithActiveTask(
	...
	if (action === "waitForUser") {
		await journalHandoffQuestion(
			workspaceRoot,
			handoff.id || path.basename(fullPath),
			"Aktif bir Cline görevi var. Mevcut görevi bitirip yenisini başlatmak ister misiniz?",
		).catch(() => {})
	...
	if (delivery === "clipboard") {
		...
		await journalHandoffPlan(workspaceRoot, handoff).catch(() => {})
		await journalHandoffActivityStart(workspaceRoot, handoff).catch(() => {})
		vscode.window.showInformationMessage(
