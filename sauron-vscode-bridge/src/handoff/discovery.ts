...
export async function readHandoffFile(fullPath: string): Promise<SauronHandoff> {
	...
	const parsed = JSON.parse(raw) as SauronHandoff & { _checksum?: string }
	...
	if (_checksum) {
		...
		if (computed !== _checksum) {
			...
			vscode.window.showWarningMessage(
				`Sauron handoff dosyası bozuk (checksum uyuşmazlığı): ${path.basename(fullPath)}. Dosya reddediliyor.`,
			)
			// Reject the file so it won't be processed again
			await markHandoffRejected(fullPath).catch(() => {})
			throw new Error(`Handoff checksum mismatch: ${path.basename(fullPath)}`)
		}
	}
	// Legacy files without _checksum are accepted (no checksum to verify)

	return content as SauronHandoff
}
