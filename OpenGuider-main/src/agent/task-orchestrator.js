...
const { blendPointerWithUia, getFocusedElementBounds } = require("./micro-guide/windows-uia");
...
class TaskOrchestrator {
  ...
  async buildMicroGuidePointer(instruction, settings = {}) {
    if (!instruction?.shouldPoint || !instruction?.pointer) {
      return null;
    }
    let pointer = { ...instruction.pointer };
    if (settings.microGuideUiaEnabled !== false) {
      const uiaBounds = await getFocusedElementBounds({ enabled: true });
      if (uiaBounds) {
        pointer = blendPointerWithUia(pointer, uiaBounds);
      }
    }
    return {
      ...
      shouldPoint: true,
      uiaAssisted: Boolean(pointer.uiaAssisted),
    };
  }
  ...
  async runMicroGuideInstructTurn({ images, settings, signal }) {
    ...
    try {
      ...
      this.sessionManager.appendMicroGuideStepHistory({
        turn: turnNumber,
        instruction: instruction.chatMessage,
        hadPointer: Boolean(instruction.shouldPoint && instruction.pointer),
      });
      ...
      const pointer = await this.buildMicroGuidePointer(instruction, settings);
      this.sessionManager.setCurrentPointer(pointer);
