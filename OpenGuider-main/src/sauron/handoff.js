...
const { prependChannelTemplate } = require("./handoff-templates");
const { buildMemorySummaryBlock, recordTask } = require("./project-memory");
...
const { resolveWorkspaceHandoffMaxChars } = require("./token-ultra/channel-limit-resolver");
...
function buildHandoffPayload(sessionSnapshot, workspacePath, handoffId = generateHandoffId(), settings = {}, overrides = {}) {
  ...
  const handoffMaxChars = resolveWorkspaceHandoffMaxChars(settings);
  ...
  const projectLabel = String(
    overrides.projectLabel
    || webBrief?.companyName
    || overrides.pipelineId
    || goalSeed,
  ).trim().slice(0, 96) || null;
  ...
  if (projectLabel) {
    payload.projectLabel = projectLabel;
  }
  ...
  const ultra = applyTokenUltraToHandoff(payload, settings, { workspacePath, channel: "workspace" });
  ...
  const memoryBlock = buildMemorySummaryBlock(workspacePath, settings);
  if (memoryBlock) {
    ultra.payload.taskSummary = `${memoryBlock}\n\n${ultra.payload.taskSummary || ""}`.trim();
  }
  const channelType = ultra.payload.projectType || projectType || "generic";
  ultra.payload.taskSummary = prependChannelTemplate(ultra.payload.taskSummary, channelType);
...
function writeHandoff(workspacePath, payload, settings = {}) {
  const sauronDir = getSauronDir(workspacePath);
  ...
  try {
    ...
    trackCall({
      provider: "sauron",
      model: "workspace-handoff",
      operation: "workspace-handoff",
      channel: "workspace",
      source: "sauron-core",
      sessionId: payloadWithChecksum.sessionId || "",
      projectLabel: payloadWithChecksum.projectLabel || "",
      sourceNote: String(payloadWithChecksum.projectLabel || payloadWithChecksum.goal || "").slice(0, 120),
      costTl: 0,
      promptTokens: 0,
      completionTokens: 0,
  ...
  try {
    recordTask(workspacePath, {
      summary: String(nextPayload.goal || nextPayload.taskSummary || "").replace(/\s+/g, " ").trim().slice(0, 160),
      projectLabel: nextPayload.projectLabel,
      themeId: nextPayload.webBrief?.themeId,
