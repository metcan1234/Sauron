...
const { summarizeByAgent, buildAgentWalletSummary, summarizeAgentChannelBreakdown, getLastSpendByAgent, buildPrimaryAgentWalletHighlight } = require("./agent-usage");
const { inferChannel, summarizeByChannel, CHANNELS } = require("./tiktoken-estimator");
...
function normalizeRecord(record = {}) {
  ...
  if (projectLabel) {
    normalized.projectLabel = projectLabel;
  }
...
function summarizeSessionByChannel(entries = [], chatSessionId = "") {
  const sessionByChannel = Object.fromEntries(CHANNELS.map((id) => [id, 0]));
  const sessionId = String(chatSessionId || "").trim();
  if (!sessionId) {
    return sessionByChannel;
  }
  for (const entry of entries) {
    if (String(entry.sessionId || "") !== sessionId) {
      continue;
    }
    const channel = inferChannel(entry);
    sessionByChannel[channel] = (sessionByChannel[channel] || 0) + (Number(entry.costTl) || 0);
  }
  for (const channel of CHANNELS) {
    sessionByChannel[channel] = Number(sessionByChannel[channel].toFixed(6));
  }
  return sessionByChannel;
}

function resolveActiveProjectLabel(workspacePath = "") {
  ...
  try {
    ...
    const briefResult = loadBrief(resolved);
    if (briefResult.ok && briefResult.brief?.companyName) {
      return String(briefResult.brief.companyName).trim() || null;
    }
  } catch {
    // ignore
  }
  try {
    const { readPipelineState } = require("../build-pipeline/pipeline-state");
    const pipeline = readPipelineState(resolved);
    if (pipeline?.status === "active") {
      if (pipeline.label) {
        return String(pipeline.label).trim();
      }
      if (pipeline.pipelineId) {
        return String(pipeline.pipelineId).trim();
      }
    }
  } catch {
    // ignore
...
async function getUsageSummary(settings = {}, options = {}) {
  ...
  const lastSpendByAgent = getLastSpendByAgent(entries, settings);
  const primaryAgentWallet = buildPrimaryAgentWalletHighlight(settings, agentWallets);
  const workspacePath = String(settings.workspacePath || "").trim();
  const tokenUltraCache = workspacePath ? readTokenUltraCache(workspacePath) : null;
  const sessionByChannel = summarizeSessionByChannel(entries, chatSessionId);
  const activeProjectLabel = resolveActiveProjectLabel(workspacePath);
  const tokenUltraStats = {
    ...
    byChannel: tokenUltraCache?.savings?.byChannel || {},
  };
