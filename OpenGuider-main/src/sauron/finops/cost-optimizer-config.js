...
function mergeCostOptimizerConfig(settings = {}) {
  ...
  const routingEnabled = shouldApplyCostOptimizerRouting(settings);
  const enabled = routingEnabled && settings.finopsCostOptimizerEnabled !== false;
  ...
  return {
    ...
    budgetGovernor: {
      ...
      dailyBudgetTl,
      enabled: shouldApplyBudgetGovernor(settings, defaults) && defaults.budgetGovernor.enabled,
    },
  };
}
...
function resolveCoreModelOverlay(settings = {}) {
  if (!shouldApplyCoreModelOverlay(settings)) {
    return null;
  }
