...
const { inferChannel, CHANNELS } = require("./tiktoken-estimator");
...
function summarizeAgentChannelBreakdown(entries = [], settings = {}) {
  const byAgent = Object.fromEntries(AGENT_IDS.map((id) => [id, emptyChannelBucket()]));
  ...
  for (const entry of entries) {
    ...
    const bucket = byAgent[agentId][channel] || {
      promptTokens: 0,
      completionTokens: 0,
      spentUsd: 0,
      entryCount: 0,
    ...
    bucket.completionTokens += Math.max(0, Number(entry.completionTokens) || 0);
    bucket.spentUsd += resolveEntryCostUsd(entry, settings);
    byAgent[agentId][channel] = bucket;
  }

  return byAgent;
}

function buildAgentWalletSummary(settings = {}, byAgent = {}, agentChannelBreakdown = {}) {
  ...
  for (const agentId of AGENT_IDS) {
    ...
    agentWallets[agentId] = {
      ...
      channels: agentChannelBreakdown[agentId] || emptyChannelBucket(),
...
async function resolveAgentWalletState(settings = {}) {
  ...
  return { byAgent, agentWallets, agentChannelBreakdown };
}
...
function resolvePrimaryAgentId(settings = {}) {
  const provider = String(settings.aiProvider || "gemini").trim().toLowerCase();
  return PROVIDER_TO_AGENT[provider] || normalizeAgentId(provider) || "gemini";
}

function getLastSpendByAgent(entries = [], settings = {}) {
  const result = {};
  for (let i = entries.length - 1; i >= 0; i -= 1) {
    ...
    result[agentId] = {
      costUsd: resolveEntryCostUsd(entry, settings),
      costTl: Math.max(0, Number(entry.costTl) || 0),
      operation,
...
function buildPrimaryAgentWalletHighlight(settings = {}, agentWallets = {}) {
  const primaryId = resolvePrimaryAgentId(settings);
  const wallet = agentWallets[primaryId];
  if (!wallet || wallet.unlimited) {
    for (const agentId of CLOUD_AGENT_IDS) {
      const candidate = agentWallets[agentId];
      if (candidate && !candidate.unlimited && candidate.totalCreditUsd > 0) {
        return { agentId, wallet: candidate };
      }
    }
    return null;
  }
  return { agentId: primaryId, wallet };
}
...
module.exports = {
  ...
  summarizeByAgent,
  summarizeAgentChannelBreakdown,
  normalizeAgentWallets,
  buildAgentWalletSummary,
  getLastSpendByAgent,
  buildPrimaryAgentWalletHighlight,
