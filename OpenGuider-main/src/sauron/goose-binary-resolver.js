...
const GOOSE_CLI_PREFERRED_PATHS = [
  ...
  () => path.join(process.env.USERPROFILE || "", "OneDrive", "Desktop", "EVERYTHİNG", "Sauron Asistan", "goose-package", "goose.exe"),
  () => path.join(process.env.USERPROFILE || "", "OneDrive", "Desktop", "EVERYTHING", "goose-package", "goose.exe"),
  () => path.join(process.env.USERPROFILE || "", "OneDrive", "Desktop", "EVERYTHING", "Sauron Asistan", "goose-package", "goose.exe"),
...
function scoreBinaryCandidate(filePath, settings = {}) {
  ...
  if (lower.includes("\\openguider-main\\goose\\goose.exe")) score += 200;
  if (lower.includes("\\sauron-main\\goose\\goose.exe")) score += 190;
  if (lower.includes("\\sauron\\goose\\goose.exe")) score += 185;
...
function collectGooseBinaryCandidates(settings = {}) {
  ...
  if (!skipAutoDiscovery) {
    for (const bundled of getBundledGoosePaths()) {
      addCandidate(candidates, seen, bundled, settings);
    }
