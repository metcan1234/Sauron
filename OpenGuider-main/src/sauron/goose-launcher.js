...
const { applyTokenUltraToGooseTask } = require("./token-ultra/goose-token-ultra");
const { resolveChannelMaxChars } = require("./token-ultra/channel-limit-resolver");
const {
...
async function launchGoose({ workspacePath, taskText, settings = {}, modeOverride = null }) {
  ...
  const gooseMaxChars = resolveChannelMaxChars(settings, "goose") || DEFAULT_TASK_MAX_CHARS;
  const optimizedTask = optimizeGooseTaskText(recipeApplied.text, {
    maxChars: gooseMaxChars,
  });
  ...
  const preUltraTask = optimizedTask.text;
  ...
  const routing = modeOverride
    ...
    : await resolveGooseMode(preUltraTask, settings);

  seedGooseInstructions(canonicalWorkspace);

  const sessionId = `goose-${Date.now()}`;
  const ultraTask = applyTokenUltraToGooseTask(preUltraTask, settings, {
    workspacePath: canonicalWorkspace,
    goal: rawTask,
    sessionId,
  });
  const task = ultraTask.text;

  const handoffResult = writeGooseHandoff(canonicalWorkspace, {
    taskText: task,
    mode: routing.mode,
    sessionId,
    tokenUltra: ultraTask.tokenUltra,
  });
