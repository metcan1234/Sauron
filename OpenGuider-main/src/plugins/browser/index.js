...
class BrowserPlugin extends OpenGuiderPlugin {
  ...
  async initialize(config = {}) {
    ...
    // Register sidecar PID with channel-runtime
    const sidecarPid = this._sidecar.pid;
    if (sidecarPid && typeof sidecarPid === 'number' && sidecarPid > 0) {
      channelRuntime.registerProcess('browser', sidecarPid, {
        label: 'Browser agent sidecar (Python)',
        dependencyPath: config.scriptPath || null,
      });
    }
  }
