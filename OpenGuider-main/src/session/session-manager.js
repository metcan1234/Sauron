...
class SessionManager extends EventEmitter {
  ...
  appendMicroGuideStepHistory(entry = {}) {
    if (!this.session.microGuideSession?.active) {
      return;
    }
    const history = Array.isArray(this.session.microGuideSession.stepHistory)
      ? this.session.microGuideSession.stepHistory
      : [];
    history.push({
      turn: Number(entry.turn) || history.length + 1,
      instruction: String(entry.instruction || "").trim(),
      timestamp: entry.timestamp || new Date().toISOString(),
      hadPointer: Boolean(entry.hadPointer),
    });
    this.session.microGuideSession.stepHistory = history.slice(-12);
    this.emitUpdate();
  }
