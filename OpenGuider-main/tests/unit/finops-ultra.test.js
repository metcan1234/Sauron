...
test("finops tracking-only keeps cost optimizer routing disabled", () => {
  const { shouldApplyCostOptimizerRouting } = require("../../src/sauron/finops/tracking-mode");
  assert.equal(shouldApplyCostOptimizerRouting({ finopsTrackingOnly: true }), false);
  assert.equal(shouldApplyCostOptimizerRouting({ finopsTrackingOnly: false }), true);
});
