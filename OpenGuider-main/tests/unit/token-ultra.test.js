...
test("applyTokenUltraToHandoff uses channel-specific max chars", () => {
  const longSummary = `Goal: test\n${"x".repeat(5000)}`;
  const workspace = require("fs").mkdtempSync(require("path").join(require("os").tmpdir(), "tu-channel-"));
  try {
    const gooseResult = applyTokenUltraToHandoff({
      version: 2,
      id: "goose-id",
      channel: "goose",
      goal: "Fix login bug",
    ...
    }, { tokenUltraEnabled: true, tokenUltraGooseMaxChars: 800 });
    assert.ok(gooseResult.payload.taskSummary.length <= 900);

    const gamedevResult = applyTokenUltraToHandoff({
      version: 2,
      id: "gd-id",
      channel: "gamedev",
      goal: "Add player",
      taskSummary: longSummary,
      workspacePath: workspace,
    }, { tokenUltraEnabled: true, tokenUltraGamedevMaxChars: 1000 });
    assert.ok(gamedevResult.payload.taskSummary.length <= 1100);
  } finally {
    require("fs").rmSync(workspace, { recursive: true, force: true });
  }
});
