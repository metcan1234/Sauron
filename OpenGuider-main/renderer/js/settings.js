...
function syncChannelRoutingGuideVisibility(enabled = true) {
  const guide = document.getElementById("channel-routing-guide");
  if (guide) {
...
async function init() {
  ...
  const microGuideUiaEl = document.getElementById("microGuideUiaEnabled");
  const microGuideAutopilotEl = document.getElementById("microGuideAutopilotOnChange");
  const microGuideHistoryEl = document.getElementById("microGuideShowStepHistory");
  if (microGuideUiaEl) microGuideUiaEl.checked = settings.microGuideUiaEnabled !== false;
  if (microGuideAutopilotEl) microGuideAutopilotEl.checked = settings.microGuideAutopilotOnChange === true;
  if (microGuideHistoryEl) microGuideHistoryEl.checked = settings.microGuideShowStepHistory !== false;
  const workspaceHubEl = document.getElementById("workspaceHubEnabled");
  const projectMemoryEl = document.getElementById("projectMemoryEnabled");
  const corporateWebAutoPipelineEl = document.getElementById("corporateWebAutoPipeline");
  ...
  document.getElementById("channelRoutingGuideEnabled")?.addEventListener("change", (event) => {
    syncChannelRoutingGuideVisibility(event.target.checked);
  });
...
function formatWalletChannel(channelId) {
  return CHANNEL_LABELS[channelId] || channelId || "—";
}

function formatWalletTimestamp(value) {
  if (!value) return "—";
...
function renderAgentWalletRows(wallets = {}, summaryWallets = {}, lastSpendByAgent = {}) {
  ...
  for (const agentId of AGENT_WALLET_IDS) {
    ...
    const statusBadge = isExhausted
      ...
      : isLow
        ...
        : "";
    const lastSpend = lastSpendByAgent[agentId];
    const lastSpendLine = lastSpend
      ? `<div class="hint" style="margin-bottom:8px">Son harcama: $${Number(lastSpend.costUsd || 0).toFixed(4)} · ${formatWalletChannel(lastSpend.channel)} · ${formatWalletTimestamp(lastSpend.timestamp)}${lastSpend.operation ? ` · ${lastSpend.operation}` : ""}</div>`
      : "";
...
function initFinOpsSettings() {
  ...
  const tokenUltraWorkspaceMaxEl = document.getElementById("tokenUltraWorkspaceMaxChars");
  if (tokenUltraWorkspaceMaxEl) {
    tokenUltraWorkspaceMaxEl.value = String(settings.tokenUltraWorkspaceMaxChars ?? 4500);
  }
  const tokenUltraGooseMaxEl = document.getElementById("tokenUltraGooseMaxChars");
  if (tokenUltraGooseMaxEl) tokenUltraGooseMaxEl.value = String(settings.tokenUltraGooseMaxChars ?? 4000);
  const tokenUltraGamedevMaxEl = document.getElementById("tokenUltraGamedevMaxChars");
  if (tokenUltraGamedevMaxEl) tokenUltraGamedevMaxEl.value = String(settings.tokenUltraGamedevMaxChars ?? 4500);
  const tokenUltraGamedevTaskMaxEl = document.getElementById("tokenUltraGamedevTaskMaxChars");
  if (tokenUltraGamedevTaskMaxEl) {
    tokenUltraGamedevTaskMaxEl.value = String(settings.tokenUltraGamedevTaskMaxChars ?? 600);
  }
  const gamedevBriefMaxCharsEl = document.getElementById("gamedevBriefMaxChars");
  if (gamedevBriefMaxCharsEl) gamedevBriefMaxCharsEl.value = String(settings.gamedevBriefMaxChars ?? 8000);
...
function bindFinOpsControls() {
  ...
  document.getElementById("btn-three-channel-economy-preset")?.addEventListener("click", () => {
    applyThreeChannelEconomyPreset();
  });
}
...
const FINOPS_PRESET_KEYS = [
  ...
  "tokenUltraWorkspaceMaxChars",
  "tokenUltraGooseMaxChars",
  "tokenUltraGamedevMaxChars",
  "tokenUltraGamedevTaskMaxChars",
  "tokenUltraMaxHandoffChars",
  "gamedevBriefMaxChars",
];
...
function applyThreeChannelEconomyPreset() {
  if (!settings.finopsPresetBackup || !Object.keys(settings.finopsPresetBackup).length) {
    settings.finopsPresetBackup = collectFinOpsPresetSnapshot();
  }
  settings.tokenUltraWorkspaceMaxChars = 3500;
  settings.tokenUltraGooseMaxChars = 3000;
  settings.tokenUltraGamedevMaxChars = 3500;
  settings.tokenUltraGamedevTaskMaxChars = 500;
  settings.gamedevBriefMaxChars = 6000;
  settings.tokenUltraEnabled = true;
  settings.tokenUltraUseDeltaHandoff = true;
  settings.tokenUltraUseRepoMap = true;
  settings.tokenUltraUseSceneCache = true;
  settings.tokenUltraSandboxToolOutput = true;
  settings.finopsTrackingOnly = true;
  initFinOpsSettings();
  showToast("Üç Kanal Ekonomi preset uygulandı — Kaydet ile kalıcı yapın");
}
...
async function refreshFinOpsSummary() {
  try {
    ...
    if (breakdownEl) {
      ...
      const sessionChannelLines = Object.entries(sessionByChannel)
        .filter(([, value]) => Number(value) > 0);
      if (sessionChannelLines.length) {
        lines.push("Oturum kanal:");
        for (const [key, value] of sessionChannelLines) {
          lines.push(`  ${CHANNEL_LABELS[key] || key}: ${Number(value).toFixed(4)} TL`);
        }
      }
      if (summary?.activeProjectLabel) {
        lines.push(`Aktif proje: ${summary.activeProjectLabel}`);
      }
    ...
    renderAgentWalletRows(collectAgentWallets(), summary?.agentWallets || {}, summary?.lastSpendByAgent || {});
  } catch (error) {
    const hintEl = document.getElementById("finopsSummaryHint");
...
async function saveSettings() {
  ...
  const newSettings = {
    ...
    microGuideUiaEnabled:    document.getElementById("microGuideUiaEnabled")?.checked !== false,
    microGuideAutopilotOnChange: document.getElementById("microGuideAutopilotOnChange")?.checked === true,
    microGuideShowStepHistory: document.getElementById("microGuideShowStepHistory")?.checked !== false,
    workspaceHubEnabled: document.getElementById("workspaceHubEnabled")?.checked !== false,
    projectMemoryEnabled: document.getElementById("projectMemoryEnabled")?.checked !== false,
    corporateWebAutoPipeline: document.getElementById("corporateWebAutoPipeline")?.checked !== false,
    ...
    tokenUltraWorkspaceMaxChars: Number(document.getElementById("tokenUltraWorkspaceMaxChars")?.value) || 4500,
    tokenUltraGooseMaxChars: Number(document.getElementById("tokenUltraGooseMaxChars")?.value) || 4000,
    tokenUltraGamedevMaxChars: Number(document.getElementById("tokenUltraGamedevMaxChars")?.value) || 4500,
    tokenUltraGamedevTaskMaxChars: Number(document.getElementById("tokenUltraGamedevTaskMaxChars")?.value) || 600,
    gamedevBriefMaxChars: Number(document.getElementById("gamedevBriefMaxChars")?.value) || 8000,
