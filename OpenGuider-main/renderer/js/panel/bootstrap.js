...
import { refreshWorkspaceHub, maybeOfferPreviewAfterClineComplete } from "./workspace-hub.js";
...
export function createPanelController({
  ...
  const messaging = createMessagingController({
    api, doc, dom, log, state, ui, webStudio, selfBuildStudio, agentTrace,
  });
  ...
  const clinePreviewOffered = { current: false };
  ...
  async function refreshHandoffHistory() {
    try {
      ...
      const hub = await refreshWorkspaceHub({
        api,
        ui,
        settings,
  ...
  async function openWorkspaceHandoff() {
    ...
    try {
      ...
      if (result.handoffFileName && result.workspacePath) {
        agentTrace?.startTrace("Çalışma Kısmı handoff");
        agentTrace?.addTodo("Handoff dosyası yazıldı", true);
        agentTrace?.addStep("Bridge bekleniyor", result.handoffFileName, { indent: true });
        await clineActivityFeed.showHandoffPlan(result.handoffFileName);
  ...
  function setGooseUiActive(active, payload = {}) {
    ...
    if (active) {
      channelControls?.onGooseSessionStarted();
    }
  }
  ...
  function setGamedevUiActive(active, payload = {}) {
    ...
    if (active) {
      channelControls?.onGamedevSessionActive();
    } else if (payload?.forceDeactivate === true || payload?.modeActive === false) {
      channelControls?.onGamedevModeEnded();
    }
  }
  ...
  function updateMicroGuideActionVisibility(sessionSnapshot = null) {
    ...
    ui.renderMicroGuideStepHistory(micro);
    syncMicroGuideAutopilot(snapshot);
  }

  function syncMicroGuideAutopilot(sessionSnapshot = null) {
    const snapshot = sessionSnapshot || state.getSessionSnapshot();
    const micro = snapshot?.microGuideSession;
    const settings = state.getSettings() || {};
    const shouldPoll = settings.microGuideAutopilotOnChange === true
      && Boolean(micro?.active)
      && micro?.status === "waiting_user";
    if (microGuideAutopilotTimer) {
      win.clearInterval(microGuideAutopilotTimer);
      microGuideAutopilotTimer = null;
    }
    if (shouldPoll && messaging.pollMicroGuideAutopilot) {
      microGuideAutopilotTimer = win.setInterval(() => {
        void messaging.pollMicroGuideAutopilot();
      }, 2500);
    }
  }
  ...
  function injectBrowserExecutionNotice(previousExecution, nextExecution) {
    ...
    if (!wasActive && isActive) {
      ...
      ui.injectSystemNotice(`⬡ ${t("browserAutomationStarted")} · ${trust.noticeLabel}`, "start");
      return;
    }
    ...
    const prefix = nextStatus === "success" ? `⬡ ${t("browserAutomationDone")}` : `⬡ ${t("browserAutomationFailed")}`;
    ui.injectSystemNotice(
      `${prefix} · ${t("browserAutomationSteps", { count: stepCount })} · ${summary}`,
  ...
  const channelControls = createChannelControls({
    ...
    routeGooseSend: (text) => {
      ...
      return openGooseSession();
    },
    routeGamedevSend: (text) => {
      if (dom.textInput) {
        dom.textInput.value = text;
      }
      if (dom.gamedevMasterPrompt && !resolveGamedevMasterPrompt()) {
        dom.gamedevMasterPrompt.value = text;
      }
      return openGamedevSession();
    },
  });
  ...
  function setupIPCListeners() {
    ...
    api.on("settings-changed", (nextSettings) => {
      ...
      void channelControls.refreshChannelStatus(api, state);
    });
    ...
    api.on("plan-updated", (plan) => {
      ...
      if (plan?.goal && agentTrace.enabled()) {
        agentTrace.startTrace(plan.goal);
        agentTrace.addTodo(`Plan: ${plan.goal}`, true);
        if (Array.isArray(plan.steps)) {
          plan.steps.slice(0, 5).forEach((step, index) => {
            const status = step.status === "done" ? true : index === plan.currentStepIndex;
            agentTrace.addTodo(`${index + 1}. ${step.title || step.instruction || "Adım"}`, status);
          });
        }
      }
  ...
  async function init() {
    ...
    try {
      ...
      if (!settings?.onboardingCompleted) {
        ui.showOnboarding();
        await runEnhancedOnboarding({ api, doc, settings });
      }
