...
import { buildFinOpsBadgeTooltip, formatFinOpsBadgeText } from "./finops-badge-format.js";
...
export function queryPanelDom(doc = document) {
  return {
    ...
    gooseBadge: doc.getElementById("goose-badge"),
    btnGooseCancel: doc.getElementById("btn-goose-cancel"),
    btnGamedev: doc.getElementById("btn-gamedev"),
    gamedevBadge: doc.getElementById("gamedev-badge"),
    ...
    microGuideStepHistory: doc.getElementById("micro-guide-step-history"),
    microGuideStepHistoryList: doc.getElementById("micro-guide-step-history-list"),
...
export function createPanelUI({ api, doc = document, dom, log, state }) {
  ...
  const clineFeedActivityElements = new Map();

  function getClineFeedKindLabel(kind) {
    const labels = {
      plan: t("clineFeedPlan"),
      activity: t("clineFeedActivity"),
      report: t("clineFeedReport"),
      question: t("clineFeedQuestion"),
    };
    return labels[kind] || kind;
  }

  function appendClineFeedMessage({ kind, title = "", body = "", eventId = "", scopeKey = "" } = {}) {
    if (!dom.chatMessages || !kind) {
      return null;
    }

    const normalizedKind = String(kind);
    if (normalizedKind === "activity" && scopeKey) {
      const existing = clineFeedActivityElements.get(scopeKey);
      if (existing && existing.isConnected) {
        const bodyEl = existing.querySelector(".cline-feed-body");
        const titleEl = existing.querySelector(".cline-feed-title-text");
        if (titleEl) {
          titleEl.textContent = title || getClineFeedKindLabel("activity");
        }
        if (bodyEl) {
    ...
    if (eventId) {
      const duplicate = dom.chatMessages.querySelector(`[data-cline-event-id="${eventId}"]`);
      if (duplicate) {
        return duplicate;
      }
    }
    ...
    const messageElement = doc.createElement("div");
    messageElement.className = `cline-feed cline-feed-${normalizedKind}`;
    if (eventId) {
      messageElement.dataset.clineEventId = eventId;
    }
    if (scopeKey) {
      messageElement.dataset.clineScopeKey = scopeKey;
    }

    const card = doc.createElement("div");
    card.className = "cline-feed-card";

    const header = doc.createElement("div");
    header.className = "cline-feed-header";

    const badge = doc.createElement("span");
    badge.className = "cline-feed-badge";
    badge.textContent = getClineFeedKindLabel(normalizedKind);

    header.appendChild(badge);

    if (title) {
      const titleEl = doc.createElement("span");
      titleEl.className = "cline-feed-title-text";
    ...
    if (body) {
      ...
      bodyEl.className = "cline-feed-body";
      bodyEl.textContent = body;
    ...
    if (normalizedKind === "activity" && scopeKey) {
      clineFeedActivityElements.set(scopeKey, messageElement);
    }
  ...
  function beginAgentTrace() {
    ...
    shell.className = "agent-trace";

    const thought = doc.createElement("div");
    thought.className = "agent-trace-thought";
    thought.textContent = t("agentTraceThought", { seconds: 1 });

    const plan = doc.createElement("div");
    plan.className = "agent-trace-plan";

    const steps = doc.createElement("div");
    steps.className = "agent-trace-steps";

    shell.appendChild(thought);
    shell.appendChild(plan);
    shell.appendChild(steps);
    dom.chatMessages.appendChild(shell);
    updateChatBackgroundState(dom.chatMessages.childElementCount);
    scrollToBottom();

    return { shell, thoughtEl: thought, planEl: plan, stepsEl: steps };
  }

  function setAgentTracePlan(trace, text) {
    if (!trace?.planEl) {
      return;
    }
    trace.planEl.textContent = String(text || "");
  }

  function appendAgentTraceTodo(trace, text, done = false) {
    if (!trace?.stepsEl) {
      return;
    }
    const row = doc.createElement("div");
    row.className = `agent-trace-todo${done ? " is-done" : ""}`;
    row.innerHTML = `<span class="agent-trace-todo-icon">${done ? "✓" : "○"}</span><span>${escapeHtml(text)}</span>`;
    trace.stepsEl.appendChild(row);
    scrollToBottom();
  }

  function appendAgentTraceStep(trace, label, detail = "", { indent = false } = {}) {
    if (!trace?.stepsEl) {
      return;
    }
    const row = doc.createElement("div");
    row.className = `agent-trace-step${indent ? " is-indent" : ""}`;
    const detailText = detail ? ` · ${detail}` : "";
    row.textContent = `${label}${detailText}`;
    trace.stepsEl.appendChild(row);
    scrollToBottom();
  }

  function appendAgentTraceQuestion(trace, text) {
    if (!trace?.stepsEl) {
      return;
    }
    const row = doc.createElement("div");
    row.className = "agent-trace-question";
  ...
  function renderHandoffHistory(items = [], options = {}) {
    ...
    const handoffStatusTr = {
      pending: t("handoffStatusPending"),
      consumed: t("handoffStatusConsumed"),
      rejected: t("handoffStatusRejected"),
      unknown: t("handoffStatusUnknown"),
    };
    ...
    dom.handoffHistoryList.innerHTML = items.map((item) => {
      ...
      const status = escapeHtml(handoffStatusTr[rawStatus] || rawStatus);
      ...
      const rejectBtn = rawStatus === "pending"
        ? `<button type="button" class="workspace-status-btn ghost handoff-reject-btn" data-handoff-file="${fileName}">${t("handoffReject")}</button>`
        : "";
      return `<li class="handoff-history-item ${rawStatus}"><div><strong>${fileName}</strong><span class="handoff-history-meta">${createdAt} · ${status}${goal ? ` · ${goal}` : ""}</span></div>${rejectBtn}</li>`;
    }).join("");
  ...
  function renderFinOpsBadge(summary) {
    ...
    const { text, hidden } = formatFinOpsBadgeText(summary, {
      tokenUltraShowDashboard: settings.tokenUltraShowDashboard !== false,
    });
    ...
    dom.finopsBadge.textContent = text;
    const baseTitle = budget <= 0
      ? t("finopsSessionNoBudget", {
        session: Number(summary?.sessionSpentTl || 0).toFixed(2),
        total: Number(summary?.totalSpentTl || 0).toFixed(2),
      })
      : t("finopsSessionBudget", {
        session: Number(summary?.sessionSpentTl || 0).toFixed(2),
        total: Number(summary?.totalSpentTl || 0).toFixed(2),
        pct: Number.isFinite(remainingPct) ? Math.round(remainingPct) : 0,
      });
    dom.finopsBadge.title = buildFinOpsBadgeTooltip(summary, baseTitle);
    dom.finopsBadge.classList.toggle(
      "is-warning",
      (Number.isFinite(remainingPct) && remainingPct <= 20)
        || (wallet?.remainingPct != null && wallet.remainingPct <= 20),
    );
    dom.finopsBadge.classList.toggle(
      "is-danger",
      (Number.isFinite(remainingPct) && remainingPct <= 5)
        || (wallet?.remainingPct != null && wallet.remainingPct <= 5),
    );
  }
  ...
  function renderMicroGuideBadge(microGuideSession) {
    ...
    renderMicroGuideStepHistory(microGuideSession);
  }

  function renderMicroGuideStepHistory(microGuideSession) {
    const settings = state.getSettings() || {};
    const show = settings.microGuideShowStepHistory !== false;
    const container = dom.microGuideStepHistory;
    const list = dom.microGuideStepHistoryList;
    if (!container || !list) {
      return;
    }
    const active = Boolean(microGuideSession?.active);
    const history = Array.isArray(microGuideSession?.stepHistory) ? microGuideSession.stepHistory : [];
    if (!active || !show || history.length === 0) {
    ...
    list.innerHTML = history
      ...
      .reverse()
      .map((step) => {
        const text = escapeHtml(String(step.instruction || "").slice(0, 96));
        const pointerMark = step.hadPointer ? " 🎯" : "";
        return `<li><span class="micro-guide-step-turn">${step.turn}.</span> ${text}${pointerMark}</li>`;
      })
      .join("");
  }
  ...
  return {
    ...
    appendErrorMessage,
    injectSystemNotice,
    appendClineFeedMessage,
    beginAgentTrace,
    setAgentTracePlan,
    appendAgentTraceTodo,
    appendAgentTraceStep,
    appendAgentTraceQuestion,
    appendRawElement,
    appendUserMessage,
    buildModelSelector,
    clearConversation,
