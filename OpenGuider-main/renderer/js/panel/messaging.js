...
import { isSelfBuildEnabled, isWebStudioEnabled } from "./feature-visibility.js";
import { t } from "../i18n/index.js";

const SELF_BUILD_PATTERNS = [
  /\b(üretim\s+hattı|self[- ]?build|pipeline\s+başlat|çok\s+fazlı)\b/i,
  /\b(build\s+pipeline|otomatik\s+inşa|üretim\s+pipeline)\b/i,
];
...
export function createMessagingController({
  ...
  let lastMicroGuideScreenFingerprint = null;
  ...
  function fingerprintScreens(screens = []) {
    ...
    if (!first) return "";
    const b64 = String(first.base64Jpeg || "").slice(0, 120);
    return `${first.width || 0}x${first.height || 0}:${b64}`;
  }

  function detectSelfBuildIntent(text = "") {
    const trimmed = String(text || "").trim();
    if (!trimmed) return false;
    return SELF_BUILD_PATTERNS.some((pattern) => pattern.test(trimmed));
  }
  ...
  async function maybeSuggestWebStudio(rawText) {
    ...
    if (Boolean(state.getSessionSnapshot()?.microGuideSession?.active)) {
      return false;
    }
    try {
      ...
      const strongIntent = Number(intent.confidence) >= 0.67;
      if (!strongIntent) {
        const open = await ui.confirmDialog({
          title: "Kurumsal site",
          message: "Bu görev için Web Studio sihirbazı açılsın mı? Tamamlandığında Cline üzerinden workspace'te devam edilir.",
          confirmLabel: "Sihirbazı aç",
      ...
      await webStudio.openWizard({ preset: "corporate" });
      return true;
    } catch (error) {
  ...
  async function maybeRouteSelfBuild(rawText) {
    if (!rawText || isGuideMode() || !selfBuildStudio?.openWizard) {
      return false;
    }
    if (!isSelfBuildEnabled(state.getSettings())) {
      return false;
    }
    if (Boolean(state.getSessionSnapshot()?.microGuideSession?.active)) {
      return false;
    }
    if (!detectSelfBuildIntent(rawText)) {
      return false;
    }
    const open = await ui.confirmDialog({
      title: "Self-Build Pipeline",
      message: "Çok fazlı üretim hattı sihirbazı açılsın mı? Pipeline Cline workspace üzerinden ilerler.",
      confirmLabel: "Pipeline başlat",
      cancelLabel: "Sohbete devam",
    });
    if (!open) {
      return false;
    }
    await selfBuildStudio.openWizard();
    return true;
  }
  ...
  async function startMicroGuideSession(goal) {
    ...
    let screens;
    try {
      screens = await api.invoke("capture-screenshot", { forceFresh: true, mode: "micro-guide" });
      if (!screens?.length) {
    ...
    lastMicroGuideScreenFingerprint = fingerprintScreens(screens);
  ...
  async function ackMicroGuide() {
    ...
    const fingerprint = fingerprintScreens(screens);
    const settings = state.getSettings() || {};
    const autopilotOn = settings.microGuideAutopilotOnChange === true;
    if (autopilotOn && lastMicroGuideScreenFingerprint && fingerprint === lastMicroGuideScreenFingerprint) {
      ui.showToast("Ekran henüz değişmedi — adımı tamamlayıp tekrar deneyin.", false);
      return;
    }
    lastMicroGuideScreenFingerprint = fingerprint;

    await runMicroGuideIpc("micro-guide-ack", { images: screens });
  }

  async function pollMicroGuideAutopilot() {
    const settings = state.getSettings() || {};
    if (settings.microGuideAutopilotOnChange !== true) {
      return;
    }
    const micro = state.getSessionSnapshot()?.microGuideSession;
    if (!micro?.active || micro.status !== "waiting_user" || state.isStreaming()) {
      return;
    }
    try {
      const screens = await api.invoke("capture-screenshot", { forceFresh: true, mode: "micro-guide" });
      if (!screens?.length) {
        return;
      }
      const fingerprint = fingerprintScreens(screens);
      if (!lastMicroGuideScreenFingerprint || fingerprint === lastMicroGuideScreenFingerprint) {
        return;
      }
      lastMicroGuideScreenFingerprint = fingerprint;
      ui.showToast("Ekran değişti — otomatik sonraki adım", false);
      await runMicroGuideIpc("micro-guide-ack", { images: screens });
    } catch (error) {
      log("micro-guide autopilot error", error);
    }
  }

  async function routeOutgoingMessage(rawText, text, options) {
    agentTrace?.startTrace(rawText);
    agentTrace?.addStep(t("agentTraceExploring"));
    agentTrace?.addStep(t("agentTraceAnalyzing"), "mikro rehber", { indent: true });

    const intent = await api.invoke("detect-micro-guide-intent", { text: rawText });
    agentTrace?.addStep(t("agentTraceAnalyzing"), "kod görevi", { indent: true });
    ...
    if (routeResult?.route) {
      agentTrace?.addRouteStep(routeResult.route);
    }
    ...
    if (routeResult?.route === "code_agent") {
      agentTrace?.addQuestion("Bu görev yerel kod agent ile workspace'te çalıştırılsın mı?");
    ...
    if (routeResult?.route === "micro_guide") {
      agentTrace?.addQuestion("Ekran rehberliği başlatılsın mı?");
      const confirmed = await ui.confirmDialog({
  ...
  async function sendAssistantMessage(text, options = {}, rawText = "") {
    ...
    if (await maybeRouteSelfBuild(displayText)) {
      agentTrace?.endTrace();
      return;
    }
    if (await maybeSuggestWebStudio(displayText)) {
      agentTrace?.endTrace();
      return;
    }
    ...
    try {
      ...
      if (!options.skipUserPersist) {
        agentTrace?.startTrace(displayText);
      }
      agentTrace?.addStep(t("agentTraceSending"));
      agentTrace?.addTodo("Sauron Core yanıtı bekleniyor", false);
      ...
      agentTrace?.addTodo("Sauron Core yanıtı bekleniyor", true);
    } catch (error) {
      onAIError(error.message);
    } finally {
      agentTrace?.endTrace();
