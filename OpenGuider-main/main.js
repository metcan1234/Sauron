...
const channelRuntime = require("./src/sauron/channel-runtime");
...
function registerChannelRuntimeIpc() {
  ipcMain.handle("channel-runtime-get-state", (_event, channelId) => {
    return channelRuntime.getState(channelId);
  });
  ipcMain.handle("channel-runtime-get-all-states", () => {
    return channelRuntime.getAllStates();
  });
  ipcMain.handle("channel-runtime-is-alive", (_event, channelId) => {
    return channelRuntime.isAlive(channelId);
  });
  ipcMain.handle("channel-runtime-register", (_event, channelId, pid, meta) => {
    channelRuntime.registerProcess(channelId, pid, meta);
    return { ok: true };
  });
  ipcMain.handle("channel-runtime-unregister", (_event, channelId) => {
    channelRuntime.unregisterProcess(channelId);
    return { ok: true };
  });
  ipcMain.handle("channel-runtime-kill", async (_event, channelId) => {
    const result = await channelRuntime.killChannel(channelId);
    return result;
  });
}
...
function setupIPC() {
  ...
  ipcMain.handle("capture-screenshot", async (_event, options = {}) => {
    ...
    debugLog("ipc:capture-screenshot", { forceFresh, mode });
    try {
      const result = await captureAllScreens({
        includeTimings: true,
        forceFresh,
        maxAgeMs: forceFresh ? 0 : 900,
        mode,
        preferActiveWindow: options?.preferActiveWindow,
...
app.whenReady().then(async () => {
  ...
  if (workspacePath) {
    channelRuntime.rotateAuditLog(workspacePath);
    channelRuntime.sweepStaleSessions(workspacePath).then((result) => {
      if (result.cleaned > 0 || result.recovered > 0) {
        appLogger.info('stale-session-sweep', result);
      }
    }).catch((err) => {
      appLogger.warn('stale-session-sweep-error', { error: err?.message });
    });
  }
  ...
  // ── FinOps logs.jsonl filewatcher for auto badge refresh ──────────────
  const finOpsLogPath = (() => {
    const wp = String(store.get("workspacePath") || "").trim();
    if (!wp) return null;
    const p = path.join(wp, '.sauron', 'usage', 'logs.jsonl');
    try { fs.mkdirSync(path.dirname(p), { recursive: true }); } catch { /* ignore */ }
    return p;
  })();
  if (finOpsLogPath) {
    try {
      fs.watchFile(finOpsLogPath, { interval: 5000 }, (cur, prev) => {
        if (cur.mtimeMs === prev.mtimeMs) return;
        appLogger.debug('finops-logs-changed');
        // Notify panel to refresh badge
        if (panelWindow && !panelWindow.isDestroyed()) {
          panelWindow.webContents.send('finops-logs-changed');
        }
        if (widgetWindow && !widgetWindow.isDestroyed()) {
          widgetWindow.webContents.send('finops-logs-changed');
        }
      });
    } catch (err) {
      appLogger.warn('finops-filewatcher-error', { error: err?.message });
    }
  }
